...
async function projectAccess(projectId, userId) {
  const result = await query(
    `SELECT p.id, p.workspace_id, p.key FROM projects p
     JOIN workspace_members wm ON wm.workspace_id = p.workspace_id
     WHERE p.id = $1 AND wm.user_id = $2`, [projectId, userId]);
  return result.rows[0] ?? null;
}
async function userIsMember(workspaceId, userId) {
  const result = await query(`SELECT 1 FROM workspace_members WHERE workspace_id = $1 AND user_id = $2`, [workspaceId, userId]);
  ...
  try {
    ...
    const issue = await withTransaction(async (client) => {
      ...
      const result = await client.query(
        `INSERT INTO issues (project_id, issue_number, title, description, priority, reporter_id, assignee_id, due_at)
         VALUES ($1,$2,$3,$4,$5,$6,$7,$8)
         RETURNING id, project_id, issue_number, title, description, status, priority, reporter_id, assignee_id, due_at, created_at, updated_at`,
        [project.id, next.rows[0].issue_number, body.data.title, body.data.description ?? null, body.data.priority, req.auth.sub, body.data.assigneeId ?? null, body.data.dueAt ?? null]);
...
router.get('/issues/:issueId', async (req, res, next) => {
  ...
  try {
    const result = await query(`SELECT i.*,p.name AS project_name,p.key AS project_key,p.workspace_id,reporter.name AS reporter_name,assignee.name AS assignee_name FROM issues i JOIN projects p ON p.id=i.project_id JOIN workspace_members wm ON wm.workspace_id=p.workspace_id AND wm.user_id=$2 JOIN users reporter ON reporter.id=i.reporter_id LEFT JOIN users assignee ON assignee.id=i.assignee_id WHERE i.id=$1`, [issueId.data, req.auth.sub]);
...
router.patch('/issues/:issueId', async (req, res, next) => {
  ...
  try {
    const existing = await query(`SELECT i.id,i.project_id,p.workspace_id FROM issues i JOIN projects p ON p.id=i.project_id JOIN workspace_members wm ON wm.workspace_id=p.workspace_id AND wm.user_id=$2 WHERE i.id=$1`, [issueId.data, req.auth.sub]);
    const current = existing.rows[0];
    if (!current) return res.status(404).json({ error: 'Issue not found' });
    if (body.data.assigneeId && !(await userIsMember(current.workspace_id, body.data.assigneeId))) return res.status(400).json({ error: 'Assignee must belong to the project workspace' });
    const fields = [];
    const params = [];
    const changes = {};
    const add = (column, value, metadataKey = column) => { params.push(value); fields.push(`${column} = $${params.length}`); changes[metadataKey] = value; };
    ...
    const issue = await withTransaction(async (client) => {
      const result = await client.query(`UPDATE issues SET ${fields.join(', ')} WHERE id=$${params.length} RETURNING id,project_id,issue_number,title,description,status,priority,reporter_id,assignee_id,due_at,created_at,updated_at`, params);
