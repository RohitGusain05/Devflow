...
async function issueAccess(issueId, userId) {
  const result = await query(
    `SELECT i.id, i.project_id, p.workspace_id
     ...
     WHERE i.id = $1 AND wm.user_id = $2`, [issueId, userId]);
...
router.get('/issues/:issueId/comments', async (req, res, next) => {
  ...
  try {
    ...
    const result = await query(
      `SELECT c.id,c.issue_id,c.body,c.created_at,c.updated_at,u.id AS author_id,u.name AS author_name
       FROM issue_comments c JOIN users u ON u.id=c.author_id WHERE c.issue_id=$1 ORDER BY c.created_at ASC`, [issueId.data]);
    ...
    const comment = await withTransaction(async (client) => {
      const result = await client.query(
        `INSERT INTO issue_comments (issue_id,author_id,body) VALUES ($1,$2,$3)
         RETURNING id,issue_id,author_id,body,created_at,updated_at`, [issueId.data, req.auth.sub, body.data.body]);
      await client.query(
